...
#include <iostream>
#include <limits>
#include <random> // Include for std::shuffle and random number generation
#include <vector>
...
double calculateDistance(const City &a, const City &b) {
  return sqrt(pow(a.x_coordinate - b.x_coordinate, 2) +
              pow(a.y_coordinate - b.y_coordinate, 2));
}

double calculateTotalDistance(const vector<City> &cities,
                              const vector<int> &tour_order) {
  double total_distance = 0;
  for (size_t i = 0; i < tour_order.size() - 1; ++i) {
    total_distance +=
        calculateDistance(cities[tour_order[i]], cities[tour_order[i + 1]]);
  }
  total_distance += calculateDistance(
      cities[tour_order.back()], cities[tour_order.front()]); // Return to start
...
vector<int> createInitialTour(int num_cities) {
  ...
  for (int i = 0; i < num_cities; ++i)
    tour_order[i] = i;
  // Use std::shuffle for random shuffling
  std::shuffle(tour_order.begin(), tour_order.end(),
...
bool shouldAcceptSolution(double current_distance, double new_distance,
                          double temperature) {
...
void simulatedAnnealing(vector<City> &cities, int num_iterations,
                        double initial_temperature, double cooling_factor) {
...
int main() {
  ...
  vector<City> cities = {{0, 0}, {1, 3}, {4, 3}, {6, 1}, {3, 0}};

  int num_iterations = 10000;
  double initial_temperature = 1000.0;
  double cooling_factor = 0.995;

  simulatedAnnealing(cities, num_iterations, initial_temperature,
                     cooling_factor);
